...
"材质GUI脚本优化“读档 ▾”为菜单模式，所有材质GUI添加“预设 ▾”菜单列。",
                "■ v2.9.5",
"[Material] 菜单添加卡通材质CustomToon。",
...
"BlendShapeAnimator v1.3 （BlendShaper混合变形顶点定位脚本）支持 SkinnedMeshRenderer 和普通 MeshFilter 模型的顶点追踪。",
...
"FPS5.0 优化算法：指数移动平均（EMA）对 1/unscaledDeltaTime 做平滑。",
                "■ v2.8.1",
"【场景工具2.17】添加【↓】快速统一赋予最后选中对象的材质按钮；添加模型一键落地按钮，以碰撞体落地操作（Ctrl+点击：以模型底部落地）。",
"【材质查找1.4】支持按住Ctrl键加选查找到的模型与材质球。",
"PBR_Mobile v6.1 添加变色通道控制，MRA贴图的a通道作为基础颜色蒙版；优化自定义聚光灯提高刷新率保证视觉效果的流畅性。",
...
"FurShell v1.4毛发材质添加GUI控制，修复风场脚本圆锥体角度压扁Bug。",
"PBR_Mobile v6.0 完善所有效果，继承原始表现效果；添加[统一阴影]按钮，用于统一设置“自身阴影衰减”值，使场景中阴影保持一致的明暗度（包括PBR_Mobile_Trans）。",
                "■ v2.7.10",
...
"添加EmissionFlicker v1.0 PBR_Mobile自发光闪烁脚本。",
"添加\"Custom\\Texture\"天空盒shader。",
                "■ v2.7.8",
"【性能分析 v1.7】 对象统计模块添加静态对象统计快速选择。",
"PBR_Mobile v5.8 优化高光亮度，移除specularColor削减；烘焙高光受实时阴影影响；添加【存档】【读档】【重置参数】按钮还原所有参数默认值，重置参数按钮默认读取Default存档（如果覆盖该存档则重置参数会读取Default文件中的设置）。",
"工具主窗口左上角添加【Menu】辅助功能菜单，包含（校正(PBR_Mobile)烘焙高光方向、校正PBR_Mobile5.8高光数值）。",
                "■ v2.7.7",
"【场景工具 v2.15】 添加一键切换lighting材质可接收实时灯光(用于场景烘焙打灯时查看实时灯光效果)快速切换功能；支持PBR_Mobile_Trans材质烘焙高光校正。",
"PBR_Mobile v5.6  修复反射被烘焙光照覆盖问题，",
"PBR_Mobile v5.7 烘焙投影支持，使用Unity标准的Subtractive模式方法。",
                "■ v2.7.6",
...
"【性能分析 v1.5】 优化未使用资源扫码准确度，精确查找BuildSetting中添加场景的资源使用，添加（扫描所有场景）选项",
...
"【Compute Buffer Tool v3.4】 管理器添加SpotTexture批量设置所有PBR_Mobile材质参数",
                ...
                "■ v2.0 改版Package管理及更新",
                "■ v1.4.8 【材质查找 v1.3】优化UI界面",
                "■ v1.4.7 【场景工具 v2.6】添加（校正(PBR_Mobile)烘焙高光方向）按钮",
                "■ v1.4.0 【场景工具 v2.5】优化资源箱列表在场景对象需要刷新时保留对象名显示；【性能分析1.4】资源利用率检查 (测试)",
                "■ v1.3.9 【场景工具 v2.4】添加层级操作按钮",
                "■ v1.3.8 【资源工具 v1.3】优化批量重命名资源对象时的安全性；【材质查找1.2】添加（查找所有Shader）按钮",
                "■ v1.3.6 添加全局光照对象检查，添加信息显示选项优化性能分析界面",
                "■ v1.3.5 重启引擎保留窗口停靠，材质查找列表添加赋予按钮；添加独立窗口；优化设置贴图参数",
                "■ v1.3.3 添加窗口停靠设置，优化其它工具",
                "■ v1.3.2 【场景工具 v2.1】修复资源箱Bug，添加选中对象标记；其它优化",
                ""
            });
